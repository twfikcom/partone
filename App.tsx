...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Reset Animation Trigger on Language Change
    const elements = document.querySelectorAll('.animate-on-scroll');
    elements.forEach((el) => {
        // We briefly remove to re-trigger if needed, though IntersectionObserver handles visibility
        // Mostly we just ensure the observer is active.
    });
  }, [lang]);
  ...
  useEffect(() => {
    ...
    // Give React a moment to render the new text nodes
    setTimeout(() => {
        const elements = document.querySelectorAll('.animate-on-scroll');
  ...
  }, [lang]); // Re-run observer when language changes (content changes)
  ...
  return (
    <>
      <div className="ancient-scroll" ref={scrollRef}>
        ...
        <p className="animate-on-scroll">{t.intro}</p>

        <div className="divider animate-on-scroll">✦</div>

        <h2 className="animate-on-scroll">{t.subtitle1}</h2>
        <p className="animate-on-scroll">{t.part1}</p>

        <div className="divider animate-on-scroll">✦</div>

        <h2 className="animate-on-scroll">{t.subtitle2}</h2>
        <p className="animate-on-scroll">{t.part2}</p>
        <p className="animate-on-scroll">{t.part3}</p>

        <div className="divider animate-on-scroll">✦</div>

        <h2 className="animate-on-scroll">{t.subtitle3}</h2>
        <p className="animate-on-scroll">{t.conclusion}</p>
        
        <p className="animate-on-scroll" style={{textAlign: 'center', fontSize: '0.9rem', marginTop: '50px', opacity: 0.7}}>
            <em>{t.footer}</em>
        </p>
      </div>
    </>
